...
class StornRobotAnomaly(object):
    ...
    def draw_pars(self, n=1):
        ...
        :return: sklearn ParameterSampler object producing n random samples
        ...
        grid = {
            ...
            # number of hidden units per generative or recognition layer
            'n_gen_hidden_units': [2 ** i for i in range(1, 6)],
            'n_rec_hidden_units': [2 ** i for i in range(1, 6)],

            # type of transfer function in generative or recognition part
    ...
    def load_data(self, pars):
        ...
        datafile =  '//brml.tum.de/dfs/public/Datasets/BaxterCollision/data' \
                    '/bluebear/bluebear.h5'
        ...
        X, M = tuple(
            interleave(l) for l in padzeros(x, front=False, return_mask=True))
        VX, VM = tuple(
            interleave(l) for l in padzeros(vx, front=False, return_mask=True))
        TX, TM = tuple(
            interleave(l) for l in padzeros(txs, front=False, return_mask=True))
        M, VM, TM = M[:, :, :1], VM[:, :, :1], TM[:, :, :1]

        X, M, VX, VM, TX, TM = [cast_array_to_local_type(i) for i in
                                (X, M, VX, VM, TX, TM)]
    ...
    def new_trainer(self, pars, data):
        ...
        # note the different file name format - this is just for
        ...
        m = mystorn(
            ...
            n_hiddens_recog=[pars['n_rec_hidden_units']] * pars[
                'n_recog_layers'],
        ...
        m.mode = NanGuardMode(nan_is_error=True, inf_is_error=False,
                              big_is_error=False)
        ...
        t = Trainer(
            ...
            interrupt=Any([climin.stops.OnSignal()]),
            # climin.stops.OnSignal(sig=signal.SIGBREAK)]),
            # loss_keys=['train', 'val'])
        )
    ...
    def make_report(self, pars, trainer, data):
        ...
        # fig.savefig('loss.pdf', transparent=True, frameon=False,
        #             bbox_inches='tight', pad_inches=.05)
        #
        # fig, ax = plt.subplots(1, 1, figsize=(16, 9))
        # ax.plot([i['val_loss'] for i in trainer.infos[0:]])
        # fig.savefig('val_loss.pdf', transparent=True, frameon=False,
        #             bbox_inches='tight', pad_inches=.05)
        #
        # ###################
        # # RECONSTRUCTION
        ...
        # fig.savefig('reconstruction.pdf', transparent=True, frameon=False,
        #             bbox_inches='tight', pad_inches=.05)
        #
        # fig, ax3d = plt.subplots(1,1, figsize=(15,11))
        # ax3d = fig.add_subplot(111, projection='3d')
        # ax3d.plot(X[:,0], X[:, 1], X[:, 2], 'bx')
        # fig.savefig('reconstruction3d.pdf', transparent=True, frameon=False,
        #             bbox_inches='tight', pad_inches=.05)
        #
        # ###################
        # # SAMPLING
        ...
        # shape = (X.shape[0], pars['n_latents'])
        # if pars['latent_assumption'] == 'KW':
        #     qs = np.random.uniform(size=shape).astype('float32')
        # else:  # includes case 'DiagGauss'
        #     qs = np.random.standard_normal(size=shape).astype('float32')
        #
        # S = f_generate(qs)
        #
        # alpha = 0.1
        ...
        # fig.savefig('sampling.pdf', transparent=True, frameon=False,
        #             bbox_inches='tight', pad_inches=.05)
